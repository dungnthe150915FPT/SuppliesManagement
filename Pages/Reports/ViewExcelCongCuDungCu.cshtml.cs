    ...
    public class ViewExcelCongCuDungCuModel : PageModel
    {
        ...
        public IActionResult OnGet(int year)
        {
            ...
            var hangHoas = _dbContext.HangHoas
                ...
                .Where(
                    h => h.NgayNhap >= startOfYear && h.NgayNhap <= endOfYear && h.NhomHangId == 1
                )
                .ToList();
            ...
            {
                return Content(
                    "<h3 style='color:red;text-align:center;'>Không có dữ liệu công cụ dụng cụ cho năm này</h3>",
                    "text/html"
                );
            }

            using var package = new ExcelPackage();
            var worksheet = package.Workbook.Worksheets.Add($"Công Cụ Dụng Cụ {year}");
        ...
        )
        {
            ...
            var tieude = a3Text.Add("SỔ THEO DÕI CÔNG CỤ DỤNG CỤ");
            a3Text.Add("\n"); // Line break
        ...
        )
        {
            ...
            sb.Append($"<div class='header'>SỔ THEO DÕI CÔNG CỤ DỤNG CỤ</div>");
