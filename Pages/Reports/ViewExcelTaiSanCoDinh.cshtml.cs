...
{
    public class ViewExcelTaiSanCoDinhModel : PageModel
    {
        private readonly SuppliesManagementProjectContext _dbContext;

        public ViewExcelTaiSanCoDinhModel(SuppliesManagementProjectContext dbContext)
        {
        ...
        public IActionResult OnGet(int year)
        {
            ...
            var hangHoas = _dbContext.HangHoas
                ...
                .Where(
                    h => h.NgayNhap >= startOfYear && h.NgayNhap <= endOfYear && h.NhomHangId == 4
                )
                .ToList();
            ...
            {
                return Content(
                    "<h3 style='color:red;text-align:center;'>Không có dữ liệu Tài Sản Cố Định cho năm này</h3>",
                    "text/html"
                );
            }

            using var package = new ExcelPackage();
            var worksheet = package.Workbook.Worksheets.Add($"Tài Sản Cố Định {year}");
        ...
        )
        {
            ...
            sb.Append($"<div class='header'>SỔ THEO DÕI TÀI SẢN CỐ ĐỊNH</div>");
