...
{
    public class PhuTungThayTheModel : PageModel
    {
        public PhuTungThayTheModel(SuppliesManagementProjectContext dBContext)
        {
    ...
    int? year = null)
        ...
        public IActionResult OnPostExport(int year)
        {
            ...
            var hangHoas = dBContext.HangHoas
                ...
                .Where(h => h.NgayNhap.Year == year && h.NhomHangId == 3)
                .ToList();
            ...
            worksheet.Cells["A3"].Value = $"SỔ THEO DÕI PHỤ TÙNG THAY THẾ";
            ...
            worksheet.Column(6).Width = 15; // Quyết định ghi tăng số hiệu
            worksheet.Column(7).Width = 15; // Ngày tháng tăng 
            worksheet.Column(8).Width = 15; // Quyết định ghi giảm số hiệu
            ...
            foreach (var hangHoa in hangHoas)
            {
                ...
                // worksheet.Cells[rowIndex, 7].Value = String.Format("{0:0,0}", hangHoa.TongGiaTruocThue); //Nguyên giá
